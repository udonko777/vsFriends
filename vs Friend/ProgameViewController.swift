//
//  ProgameViewController.swift
//  vs Friend
//
//  Created by 池上　魁 on 2016/07/17.
...
class ProgameViewController: UIViewController {
    ...
    var score:Int = 0
    var score_board:[Int] = []
    var random_number:Int = Int(arc4random() % 9) + 1
    var next_random_number:Int = Int(arc4random() % 9) + 1
    var combo_count:Int = 0
    let image = UIImage(named: "ボタン用")
    let image2 = UIImage(named: "ボタン用2")
    let image3 = UIImage(named: "ボタン用3")
    var message_st:String = "このメッセージは表示されないはずだよ！"
            ...
            UIAlertAction(
                ...
                handler: {action in
                    ...
                    self.timer = Timer.scheduledTimer(timeInterval: 0.01,
                        target: self,
                        selector:  #selector(ProgameViewController.up),
                        userInfo: nil,
                        repeats: true       )
                }
            )
